...
class IntegralController extends BaseController{
			...
			if(!IS_AJAX){
				...
				if($res2){
					...
					if ($type == 2) {
						...
						$this->success('Nạp成功,跳转中~',U('Admin/Fen/addlist'),1);
					}else{
						$this->success('Nạp成功,跳转中~',U('Admin/Member/index'),1);
					}
	...
	public function index2(){
		...
		$payno=$_REQUEST['payno'];//接收参数payno 一般是tên tài khoản 用户ID
	...
	public function agent_index(){
		if(IS_POST){
			if(!IS_AJAX){
				...
				if($res2){
					$this->success('Nạp成功,跳转中~',U('Admin/Member/push'),1);
				}else{
					$this->error('Nạp失败！');
				}
			}
