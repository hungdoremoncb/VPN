		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			if($next_time-time()== 30+C('jnd28_2_stop_time')){
				$new_message = array(
					...
					'content'=>'Number:'.$jnd28_2data['next']['periodNumber'].'<br/>'.'--距离封盘还有30秒--',
					'time'=>date('H:i:s')
				);
			...
			if($next_time-time()==C('jnd28_2_stop_time')){
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'Number:'.$jnd28_2data['next']['periodNumber'].'已封盘，请耐心等待结果',
					'time'=>date('H:i:s')
				);
				foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode($new_message));
			...
			if($next_time-time()<$this->sec_period && $next_time-time()>100 && F('jnd28_2_is_send')==0){
                ...
                $new_message = array(
                    ...
                    'head_img_url'=>'/Public/main/img/kefu.jpg',
                    'from_client_name' => 'GM管理员',
                    'content'=>'Number:'.$jnd28_2data['next']['periodNumber'].'开放，祝各位购买成功',
                    'time'=>date('H:i:s')
                );
