...
class Workermanpk10Controller extends Server {
	...
	protected $socket = 'websocket://103.166.184.168:15531';
	...
	public function onWorkerStart(){
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			if($next_time+900-time()>C('pk10_stop_time')-6 && $next_time+900-time()<1180 && time()>$beginToday && time()<$endToday ){
				F('pk10_state',1);
				setconfig('pk10_state',1);
			}else{
				F('pk10_state',0);
			...
			if($next_time+900-time()== 24+C('pk10_stop_time')){
				$new_message = array(
					'type' => 'admin',
			...
			if($next_time+900-time()==C('pk10_stop_time')-6){
				F('pk10_state',0);
				setconfig('pk10_state',0);
				$new_message = array(
					'type' => 'admin',
