...
class Workermanjnd28_1Controller extends Server {

	protected $socket = 'websocket://103.166.184.168:9004';
    ...
    public function onWorkerStart(){
        ...
        $time_interval = 1;
        $jnd28_1data = json_decode($data,true);
        $nexttime = $jnd28_1data['next']['delayTimeInterval']+strtotime($jnd28_1data['next']['awardTime']);
        //        $nexttime-time()>C('jnd28_1_stop_time') &&
        ...
        if(!F('jnd28_1data')){
            F('jnd28_1data',$jnd28_1data);
        }
        if(!F('jnd28_1_is_send')){
            F('jnd28_1_is_send',1);
        }
        ...
        \Workerman\Lib\Timer::add($time_interval, function(){
            ...
            $beginToday=strtotime('09:00:00');
            $endToday=strtotime("23:54:59");
            F('jnd28_1_game','jnd28_1');

            $jnd28_1data = F('jnd28_1data');
            $next_time = $jnd28_1data['next']['delayTimeInterval']+strtotime($jnd28_1data['next']['awardTime']);
            $awardtime = $jnd28_1data['current']['awardTime'];
            // var_dump($next_time-time()); $next_time-time()>C('jnd28_1_stop_time') &&
            if($next_time-time()<$this->sec_period){
                F('jnd28_1_state',1);
            }else{
                F('jnd28_1_state',0);
            }

            if($next_time-time()== 30+C('jnd28_1_stop_time')){
                $new_message = array(
                    'type' => 'admin',
                    'head_img_url'=>'/Public/main/img/kefu.jpg',
                    'from_client_name' => 'GM管理员',
                    'content'=>'期号:'.$jnd28_1data['next']['periodNumber'].'<br/>'.'--距离封盘还有30秒--',
                    'time'=>date('H:i:s')
                );
            ...
            if($next_time-time()==C('jnd28_1_stop_time')){
                F('jnd28_1_state',0);
                $new_message = array(
                    'type' => 'admin',
                    'head_img_url'=>'/Public/main/img/kefu.jpg',
                    'from_client_name' => 'GM管理员',
                    'content'=>'期号:'.$jnd28_1data['next']['periodNumber'].'已封盘，请耐心等待结果',
                    'time'=>date('H:i:s')
                );
                foreach ($this->worker->connections as $conn) {
                    $conn -> send(json_encode($new_message));
            ...
            if($next_time-time()<$this->sec_period && $next_time-time()>100 && F('jnd28_1_is_send')==0){
                //结算
                //开奖结果
                ...
                $list = M('order')->where("number = {$current_number['periodnumber']} && time > '{$today_time}' && state = 1 && is_add = 0 && game='jnd28_1'")->order("time ASC")->select();
                if($list){
                    for($i=0;$i<count($list);$i++){
                        ...
                        case 1:
                            ...
                            if($num1>0){
                                ...
                                //     $points1 = $num1*$starts1*C('jnd28_1_dxds_md');
                                // } else {
                                $points1 = $num1*$starts1*C('jnd28_1_dxds');
                                // }
                        ...
                        case 2:
                            ...
                            if($num2>0){
                                // if ($current_number['tema'] == '13' || $current_number['tema'] == '14') {
                                //     $points2 = $num2*$starts2*C('jnd28_1_zuhe_md');
                                // } else {
                                if ($start2 == '大单' || $start2 == '小双') {
                                    $points2 = $num2*$starts2*C('jnd28_1_zuhe_1');
                                } else {
                                    $points2 = $num2*$starts2*C('jnd28_1_zuhe_2');
                                }
                                // }
                        ...
                        case 3:
                            ...
                            if($num3>0){
                                $points3 = $num3*$starts3*C('jnd28_1_jdx');
                        ...
                        case 4:
                            ...
                            if ($start4 == '庄' || $start4 == '闲') {
                                if ($current_number['zx'] == '和') {
                                    ...
                                    $points4 = $starts4*C('jnd28_1_zx_1');
                                }
                            } else {
                                if($start4 == $current_number['zx']){
                                    $points4 = $starts4*C('jnd28_1_zx_2');
                                }
                            }
                            if ($points4) {
                        ...
                        case 5:
                            ...
                            if($num5>0){
                                if ($start5 == '豹子') {
                                    $points5 = $num5*$starts5*C('jnd28_1_bds_1');
                                } else if($start5 == '顺子') {
                                    $points5 = $num5*$starts5*C('jnd28_1_bds_2');
                                } else if($start5 == '对子') {
                                    $points5 = $num5*$starts5*C('jnd28_1_bds_3');
                                } else if($start5 == '半顺') {
                                    $points5 = $num5*$starts5*C('jnd28_1_bds_4');
                                } else if($start5 == '杂六') {
                                    $points5 = $num5*$starts5*C('jnd28_1_bds_5');
                                }
                        ...
                        case 6:
                            ...
                            if($num6>0){
                                $points6 = $num6*$start6[1]*C('jnd28_1_tema_'.$tema_number);
                ...
                $new_message = array(
                    ...
                    'head_img_url'=>'/Public/main/img/kefu.jpg',
                    'from_client_name' => 'GM管理员',
                    'content'=>'期号:'.$jnd28_1data['next']['periodNumber'].'开放，祝各位购买成功',
                    'time'=>date('H:i:s')
                );
        ...
        \Workerman\Lib\Timer::add($time_interval, function(){
            ...
            //机器人上下分频率
            if (F('jnd28_1_state') == 1 && C('is_robot_addfen') == '1') {
                $this->robot_rate_fen--;
        ...
        //存每期结果
		$page = 1;
        \Workerman\Lib\Timer::add(5, function() use(&$page){
            $caiji = M('caiji')->where("game='jnd28'")->order("id desc")->find();
			//var_dump($caiji);
            $data = json_decode(jnd28_format($caiji),true);
			//var_dump(F('periodNumber'));
			//var_dump($data['current']['periodNumber']);
			//var_dump(F('periodNumber')!= $data['current']['periodNumber']);
    ...
    public function onMessage($connection, $data) {
        ...
        case 'say' :
            ...
            if($jnd28_1_state == 0){
                ...
                /*检测格式和金额*/
                $res = check_format_bj28($message_data['content'],C('jnd28_1qi_min_point'),C('jnd28_1qi_max_point'),C('jnd28_1_xz_open'),C('jnd28_1_xz_max'));
                //	var_dump($res);
                if($res['error']==0 || $res['error']==2){
                    ...
                    if ($res['error'] == 0) {
                        $content_msg =  '「'.$message_data['content'].'」'.'单笔数量最低'.C('jnd28_1qi_min_point').',购买失败';
                    } else {
                    ...
                    if($jifen['points']<$res['points']){
                        ...
                        switch ($res['type']) {
                        case '1':
                            $wf_max_points = C('jnd28_1_xz_max')['dxds'];
                            break;

                        case '2':
                            $wf_max_points = C('jnd28_1_xz_max')['zuhe'];
                            break;

                        case '3':
                            $wf_max_points = C('jnd28_1_xz_max')['jdx'];
                            break;

                        case '4':
                            $wf_max_points = C('jnd28_1_xz_max')['zx'];
                            break;

                        case '5':
                            $wf_max_points = C('jnd28_1_xz_max')['bds'];
                            break;

                        case '6':
                            $wf_max_points = C('jnd28_1_xz_max')['tema'];
                            break;
                        ...
                        if ((intval($user_points['sum_del'])+$res['points']) > C('jnd28_1qi_max_point')) {
                            $points_tips = array(
                                'uid'  => $connection->uid,
                                'type' => 'admin',
                                'head_img_url'=>'/Public/main/img/kefu.jpg',
                                'from_client_name' => 'GM管理员12',
                                'content' => '「'.$message_data['content'].'」'.'每期最高点数'.C('jnd28_1qi_max_point').',您已超过，竞猜失败',
                                'time' => date('H:i:s')
                            );
                            $connection -> send(json_encode($points_tips));
                            $points_tips['type'] = 'error';
                        ...
                        //判断时候低于进入房间最低点数
                        if ($user['points'] < C('jnd28_1_usermin_point')) {
                            $points_tips = array(
                                'uid'  => $connection->uid,
                                'type' => 'admin',
                                'head_img_url'=>'/Public/main/img/kefu.jpg',
                                'from_client_name' => 'GM管理员',
                                'content' => '您的点数低于加拿大28初级房进入最低点数'.C('jnd28_1_usermin_point').',竞猜失败',
                                'time' => date('H:i:s')
                            );
                            $connection -> send(json_encode($points_tips));
                            $points_tips['type'] = 'error';
    ...
    public function robot_message($zuhe=null){
        $zuhe = $zuhe?$zuhe:1;
        $count = M('robot_message')->where("type=4 && zuhe = {$zuhe}")->count();
