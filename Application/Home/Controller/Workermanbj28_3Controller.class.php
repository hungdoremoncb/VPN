...
class Workermanbj28_3Controller extends Server {

	protected $socket = 'websocket://103.166.184.168:9003';
	...
	public function onWorkerStart(){
		...
		$time_interval = 1;
		$bj28_3data = json_decode($data,true);
		$nexttime = $bj28_3data['next']['delayTimeInterval']+strtotime($bj28_3data['next']['awardTime']);
		if($nexttime-time()>C('bj28_3_stop_time') && $nexttime-time()<$this->sec_period && time()>$beginToday && time()<$endToday){
			F('bj28_3_state',1);
		}else{
			F('bj28_3_state',0);
		}
		if(!F('bj28_3data')){
			F('bj28_3data',$bj28_3data);
		}
		if(!F('bj28_3_is_send')){
			F('bj28_3_is_send',1);
		}
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			$beginToday=strtotime('00:00:00');
			$endToday=strtotime("23:54:59");
			F('bj28_3_game','bj28_3');

			$bj28_3data = F('bj28_3data');
			$next_time = $bj28_3data['next']['delayTimeInterval']+strtotime($bj28_3data['next']['awardTime']);
			$awardtime = $bj28_3data['current']['awardTime'];
			// var_dump($next_time-time());
			if($next_time-time()>C('bj28_3_stop_time') && $next_time-time()<$this->sec_period && time()>$beginToday && time()<$endToday ){
				F('bj28_3_state',1);
			}else{
				F('bj28_3_state',0);
			}

			if($next_time-time()== 30+C('bj28_3_stop_time')){
				$new_message = array(
					'type' => 'admin',
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$bj28_3data['next']['periodNumber'].'<br/>'.'--距离封盘还有30秒--',
					'time'=>date('H:i:s')
				);
			...
			if($next_time-time()==C('bj28_3_stop_time')){
				F('bj28_3_state',0);
				$new_message = array(
					'type' => 'admin',
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$bj28_3data['next']['periodNumber'].'关闭，请耐心等待结果',
					'time'=>date('H:i:s')
				);
				foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode($new_message));
			...
			if((($next_time-time()<$this->sec_period && $next_time-time()>100) || (time() > strtotime("23:52:00") && time() < strtotime("23:55:00"))) && F('bj28_3_is_send')==0){
				//结算
				//开奖结果
				...
				$list = M('order')->where("number = {$current_number['periodnumber']} && time > '{$today_time}' && state = 1 && is_add = 0 && game='bj28_3'")->order("time ASC")->select();
				if($list){
					for($i=0;$i<count($list);$i++){
                        ...
                        case 1:
                            ...
                            if($num1>0){
								...
								// 		$points1 = $num1*$starts1*C('bj28_3_dxds_md');
								// 	} else {
                                $points1 = $num1*$starts1*C('bj28_3_dxds');
								// 	}
                        ...
                        case 2:
                            ...
                            if($num2>0){
								// 	if ($current_number['tema'] == '13' || $current_number['tema'] == '14') {
								// 		$points2 = $num2*$starts2*C('bj28_3_zuhe_md');
								// 	} else {
                                if ($start2 == '大单' || $start2 == '小双'){
                                    $points2 = $num2*$starts2*C('bj28_3_zuhe_1');
                                } else {
                                    $points2 = $num2*$starts2*C('bj28_3_zuhe_2');
                                }
								// 	}
                        ...
                        case 3:
                            ...
                            if($num3>0){
                                $points3 = $num3*$starts3*C('bj28_3_jdx');
                        ...
                        case 4:
                            ...
                            if ($start4 == '庄' || $start4 == '闲') {
                                if ($current_number['zx'] == '和') {
                                    ...
                                    $points4 = $starts4*C('bj28_3_zx_1');
                                }
                            } else {
                                if($start4 == $current_number['zx']){
                                    $points4 = $starts4*C('bj28_3_zx_2');
                                }
                            }
                            if ($points4) {
                        ...
                        case 5:
                            ...
                            if($num5>0){
                                if ($start5 == '豹子') {
                                    $points5 = $num5*$starts5*C('bj28_3_bds_1');
                                } else if($start5 == '顺子') {
                                    $points5 = $num5*$starts5*C('bj28_3_bds_2');
                                } else if($start5 == '对子') {
                                    $points5 = $num5*$starts5*C('bj28_3_bds_3');
                                } else if($start5 == '半顺') {
                                    $points5 = $num5*$starts5*C('bj28_3_bds_4');
                                } else if($start5 == '杂六') {
                                    $points5 = $num5*$starts5*C('bj28_3_bds_5');
                                }
                        ...
                        case 6:
                            ...
                            if($num6>0){
                                $points6 = $num6*$start6[1]*C('bj28_3_tema_'.$tema_number);
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$bj28_3data['next']['periodNumber'].'开放，祝各位理投成功',
					'time'=>date('H:i:s')
				);
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			//机器人上下分频率
			if (F('bj28_3_state') == 1 && C('is_robot_addfen') == '1') {
				$this->robot_rate_fen--;
	...
	public function onMessage($connection, $data) {
        ...
        case 'say' :
            ...
            if($bj28_3_state == 0){
                ...
                /*检测格式和金额*/
                $res = check_format_bj28($message_data['content'],C('bj28_3qi_min_point'),C('bj28_3qi_max_point'),C('bj28_3_xz_open'),C('bj28_3_xz_max'));
                if($res['error']==0 || $res['error']==2){
                    ...
                    if ($res['error'] == 0) {
                        $content_msg =  '「'.$message_data['content'].'」'.'单笔点数最低'.C('bj28_3qi_min_point').',竞猜失败';
                    } else {
                    ...
                    if($jifen['points']<$res['points']){
                        ...
                        //当前玩法是否超过设置金额
                        $wf_points = M('order')->field("sum(del_points) as sum_del")->where("userid = {$userid} and type={$res['type']} and state=1  and number = {$bj28_3data['next']['periodNumber']}")->find();

                        $wf_max_points = 0;
                        switch ($res['type']) {
                        case '1':
                            $wf_max_points = C('bj28_3_xz_max')['dxds'];
                            break;

                        case '2':
                            $wf_max_points = C('bj28_3_xz_max')['zuhe'];
                            break;

                        case '3':
                            $wf_max_points = C('bj28_3_xz_max')['jdx'];
                            break;

                        case '4':
                            $wf_max_points = C('bj28_3_xz_max')['zx'];
                            break;

                        case '5':
                            $wf_max_points = C('bj28_3_xz_max')['bds'];
                            break;

                        case '6':
                            $wf_max_points = C('bj28_3_xz_max')['tema'];
                            break;
                        ...
                        //查看已投注金额
                        $user_points = M('order')->field("sum(del_points) as sum_del")->where("userid = {$userid} and state=1  and number = {$bj28_3data['next']['periodNumber']}")->find();

                        if ((intval($user_points['sum_del'])+$res['points']) > C('bj28_3qi_max_point')) {
                            $points_tips = array(
                                'uid'  => $connection->uid,
                                'type' => 'admin',
                                'head_img_url'=>'/Public/main/img/kefu.jpg',
                                'from_client_name' => 'GM管理员',
                                'content' => '「'.$message_data['content'].'」'.'每期最高点数'.$res['bj28_3qi_max_points'].',您已超过，竞猜失败',
                                'time' => date('H:i:s')
                            );
                            $connection -> send(json_encode($points_tips));
                            $points_tips['type'] = 'error';
                        ...
                        //判断时候低于进入房间最低点数
                        if ($user['points'] < C('bj28_3_usermin_point')) {
                            $points_tips = array(
                                'uid'  => $connection->uid,
                                'type' => 'admin',
                                'head_img_url'=>'/Public/main/img/kefu.jpg',
                                'from_client_name' => 'GM管理员',
                                'content' => '您的点数低于北京28高级房进入最低点数'.C('bj28_3_usermin_point').',竞猜失败',
                                'time' => date('H:i:s')
                            );
                            $connection -> send(json_encode($points_tips));
                            $points_tips['type'] = 'error';
                        ...
                        $map['time'] = time();
                        $map['number'] = $bj28_3data['next']['periodNumber'];
	...
	public function robot_message($zuhe){
		$zuhe = $zuhe?$zuhe:1;
		$count = M('robot_message')->where("type=3 && zuhe = {$zuhe}")->count();
     	$rand = mt_rand(0,$count-1); //产生随机数。
     	$limit = $rand.','.'1';
    	$data = M('robot_message')->where("type=3 && zuhe = {$zuhe}")->limit($limit)->select();
		return $data;
	}
