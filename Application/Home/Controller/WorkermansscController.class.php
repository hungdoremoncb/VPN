...
use Think\Server;
//本程序来自：668源码网【668yuanma.com】
header('content-type:text/html;charset=utf-8');
class WorkermansscController extends Server {

	protected $socket = 'websocket://103.166.184.168:15533';
	...
	public function onWorkerStart(){
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			$time = time();
			if ($time < strtotime("23:57:00") && $time > strtotime("07:00:00")) {
				$st_time = 1206;
			} else {
				$st_time = 1206;
			}

			if($next_time+780-time()>C('ssc_stop_time') && $next_time+780-time()<$st_time ){
				F('ssc_state',1);
				setconfig('ssc_state',1);
			}else{
				F('ssc_state',0);
			...
			if($next_time+780-time()==30+C('ssc_stop_time')){
				$new_message = array(
					'type' => 'admin',
			...
			if($next_time+780-time()==C('ssc_stop_time')){
				F('ssc_state',0);
				setconfig('ssc_state',0);
				$new_message = array(
					'type' => 'admin',
			...
			if((($next_time-time()<$st_time && $next_time-time()>$st_time-200) || (time() > strtotime("23:54:00") && time() < strtotime("23:57:00"))) && F('is_send')==0){
				//结算
				//开奖结果
