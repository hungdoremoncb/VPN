...
class Workermanxjp28Controller extends Server {
	...
	protected $socket = 'websocket://103.166.184.168:15536';
	...
	public function onWorkerStart(){
		...
		if($nexttime-time()>C('xjp28_stop_time') && $nexttime-time()<180 && time()>$beginToday && time()<$endToday){
			F('xjp28_state',1);
		...
		\Workerman\Lib\Timer::add($time_interval, function(){
			...
			$next_time = $xjp28data['next']['delayTimeInterval']+strtotime($xjp28data['next']['awardTime'])-0;
			$awardtime = $xjp28data['current']['awardTime'];
			// var_dump($next_time-time());
			if($next_time-time()>C('xjp28_stop_time') && $next_time-time()<180 && time()>$beginToday && time()<$endToday ){
				F('xjp28_state',1);
				setconfig('xjp28_state',1);
			}else{
				F('xjp28_state',0);
			...
			if($next_time-time()==C('xjp28_stop_time')){
				...
				$new_message = array(
					...
					'head_img_url'=>'/Public/main/img/kefu.jpg',
					'from_client_name' => 'GM管理员',
					'content'=>'期号:'.$xjp28data['next']['periodNumber'].'关闭，请耐心等待开盘',
					'time'=>date('H:i:s')
				);
				foreach ($this->worker->connections as $conn) {
					$conn -> send(json_encode($new_message));
			...
			if((($next_time-time()<180 && $next_time-time()>0) || (time() > strtotime("23:52:00") && time() < strtotime("23:55:00"))) && F('is_send')==0){
				//结算
				//开盘结果
		...
		/* \Workerman\Lib\Timer::add(300, function(){
			$new_message = array(
				...
				'head_img_url'=>'/Public/main/img/system.jpg',
				'from_client_name' => 'cskh',
				'content'=>'由于各地网络情况不同，开盘动画仅作为参考，可能存在两秒的误差，不影响最终开盘结果！',
				'time'=>date('H:i:s')
			);
			foreach ($this->worker->connections as $conn) {
				$conn -> send(json_encode($new_message));
