...
fnSet.countDown=function(msg){
    ...
    if(msg.stopOrSell == 1)
    {
        if(theTime2 && theTime2 > 0){
            ...
            wsSendMsg(param);
            // $(".roomLi p:last").html('<span class="icoTime">开盘中</span>');
        }
...
fnSet.update37 =function(msg){
    ...
    } else {  //除了游客和Đăng ký tài khoản的其他
...
$(function(){
    ...
    $("body").on("click",".userBetting dd .bet-con-self, .userBetting dd .bet-con",function(){
        ...
        for(var i=0; i<genZhi.length; i++){
            gentou+='<p><span style="color: #5e97fe">'+genZhi.eq(i).children("span").text()+'</span> <label class="configAcer" style="text-align: right;"><em>'+genZhi.eq(i).find("em").text()+'</em></label></p>'
        }
