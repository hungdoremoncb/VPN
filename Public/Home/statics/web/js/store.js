...
//             }
            ...
            if(userinfo.lottery_type==12){
                ...
                roomGroup +='<li style="font-size: 10px;text-align: center;padding: 0 5px;"><p style="color:#2aa7f6 "><p>开盘时间：'+msg.open_time+'</p><p>开盘结果：'+msg.result+'</p>'; 
                roomGroup += '</li></ul></div>';
            ...
            if(userinfo.lottery_type ==4 ||userinfo.lottery_type == 9 || userinfo.lottery_type ==5 || userinfo.lottery_type == 6|| userinfo.lottery_type == 10 || userinfo.lottery_type == 11 || userinfo.lottery_type == 13){
             ...
             if(userinfo.lottery_type == 10){
                roomGroup +='<li style="font-size: 10px;text-align: left;padding: 0 5px;"><p style="color:#2aa7f6 ">期号：'+dataIssue+'</p><p>开盘时间：'+msg.open_time+'</p><p>开盘号码：'+msg.result+'</p><p>开盘结果：'+msg.sum_result_str+'</p>'; 
             }else{
                roomGroup +='<li style="font-size: 10px;text-align: left;padding: 0 5px;"><p style="color:#2aa7f6 ">期号：'+dataIssue+'</p><p>开盘时间：'+msg.open_time+'</p><p>开盘结果：'+msg.result+'</p>';
            }
            if (msg.statistics != '') {
                roomGroup += '<p>统计：</p>';
            }
			...
			if(userinfo.lottery_type ==2 ||userinfo.lottery_type == 9 || userinfo.lottery_type ==4 || userinfo.lottery_type ==5 || userinfo.lottery_type == 6 || userinfo.lottery_type == 11 || userinfo.lottery_type == 13){  //北京PK10和幸运飞艇
			    ...
			    // if(userinfo.lottery_type ==4 || userinfo.lottery_type ==5 || userinfo.lottery_type == 6){
                    ...
                    var html2 ='<ul><li class="colorjieguo_down">第<em>'+dataIssue+'</em>期 '; //幸运飞艇更新开盘结果记录
                // }else{
                //     var html = '第<label id="issue2">'+msg.issue+'</label>期 '
                //     var html2 ='<ul><li class="colorjieguo_down">第<em>'+msg.issue+'</em>期 '; //北京Pk更新开盘结果记录
                // }
                    //'<li>'+msg.open_time+'</li>' +
                ...
                $(".roomHead2 span").html('第<i id="issue2">'+msg.issue+'</i>期<em class="num_l">'+resule[0]+'</em> + <em class="num_l">'+resule[1]+'</em> + <em class="num_l">'+resule[2]+'</em> = <em class="num_r">'+resule[3]+'</em><em style="color: #2aa7f6"> ('+resule[4]+') </em>'); //幸运28更新开盘结果记录
